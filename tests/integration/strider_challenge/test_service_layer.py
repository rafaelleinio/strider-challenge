...
@pytest.mark.parametrize(
    ...
    [
        ...
        (
            ...
            adapters.JsonCollector(path=f"{DATA_FOLDER}/internal/authors.json"),
            "Josh Johnston",
        ),
        (
            model.Book,
            service_layer.load_books,
            adapters.JsonCollector(path=f"{DATA_FOLDER}/internal/books.json"),
            "An Evil Cradling",
        ),
        (
            model.Review,
            service_layer.load_reviews,
            adapters.JsonCollector(path=f"{DATA_FOLDER}/internal/reviews.json"),
