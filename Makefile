# globals
VERSION := $(shell grep __version__ strider_challenge/__metadata__.py | head -1 | cut -d \" -f2 | cut -d \' -f2)
...
type-check:
	...
	@echo "======="
	@echo ""
	@python -m mypy --install-types --non-interactive strider_challenge && echo "Success"
	@echo ""
...
.PHONY: build-docker
## build strider_challenge image
build-docker:
	@docker build -t strider_challenge:$(VERSION) \
	-t strider_challenge:latest .
...
clean:
	...
	@find ./ -type d -name 'strider_challenge.egg-info' -exec rm -rf {} +;
